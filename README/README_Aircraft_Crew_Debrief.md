...
| **To downstream**   | `crew_debrief_status`                                                                                                                                                                                 |
| **From upstream**   | `flight_number`, `aircraft_type`, `flight_status`, `gate_id`, `jetbridge_connection_status`, `stairtruck_connection_status`, `deplaning_equipment_type`, `door_opening_status`, `crew_debrief_status` |
| **From downstream** | `flight_number`, `aircraft_type`, `flight_status`, `gate_id`, `jetbridge_connection_status`, `stairtruck_connection_status`, `deplaning_equipment_type`, `door_opening_status`, `crew_debrief_status` |
|---------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
**Tracked fields:**
`aircraft_type`, `crew_debrief_status`, `deplaning_equipment_type`, `door_opening_status`, `flight_number`, `flight_status`, `gate_id`, `jetbridge_connection_status`, `passenger_disembarkation_status`, `stairtruck_connection_status`

**Return fields:**
`crew_debrief_status`, `deplaning_equipment_type`, `door_opening_status`, `flight_status`, `jetbridge_connection_status`, `passenger_disembarkation_status`, `stairtruck_connection_status`

> Note: `passenger_disembarkation_status` is tracked and returned by this network's `TrackerAPI` even though the orchestration flow does not use it as a prerequisite. It is carried for context so that upstream orchestrators can read it if needed.

> Note: The HOCON `TrackerAPI` tool definition is missing a `"required": []` field in its `parameters` object — the schema closes with just a `}` rather than the `"required": [] }` present in all other networks. This is a HOCON schema inconsistency but has no runtime impact since no fields are required.
