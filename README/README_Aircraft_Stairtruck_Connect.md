...
| `flight_status`                   | string |    ✅     | Expected: contains `on blocks` or `on-block` |
...
> Note: The agent `function.description` correctly says "I am in charge of connecting the **stairtruck** to the aircraft at the gate."
...
`flight_status` must contain `'on blocks'` or `'on-block'`. If not → stop and report: `"Cannot connect stairtruck — aircraft is not yet on blocks."`
...
> Note: `'on-block'` in STEP 2 is a narrower substring match than the prior `'block'` form, but still permits hyphenated variants like `"on-blocked"`. Same family of limitation as `aircraft_jetbridge_connect`.
...
| **To downstream**   | `flight_number`, `aircraft_type`, `gate_id`, `flight_status`, `acu_connection_status`, `gpu_connection_status`, `wheels_chocks_installation_status`            |
| **From upstream**   | `flight_number`, `aircraft_type`, `gate_id`, `flight_status`, `acu_connection_status`, `gpu_connection_status`, `wheels_chocks_installation_status`            |
| **From downstream** | `flight_number`, `aircraft_type`, `gate_id`, `acu_connection_status`, `gpu_connection_status`                                                                  |
...
> Note: The HOCON TrackerAPI schema includes both `jetbridge_connection_status` AND `stairtruck_connection_status` (lines 318, 322). `aircraft_jetbridge_connect`'s TrackerAPI only included `jetbridge_connection_status`. This wider schema is appropriate for a TrackerAPI that may be shared with door-opening networks.
...
| `stairtruck_operator` description says "connects **jetbridge**"           | `aircraft_stairtruck_connect.hocon` line 225   |   Low    | Copy-paste from `aircraft_jetbridge_connect.hocon`. Should say "stairtruck".                                                                                                                                                           |
...
| Agent description copy-paste                        | No                            | No (description correctly says "stairtruck")                                |
