...
> Note: `crew_exit_status` is consistently declared as a quoted property key (`"crew_exit_status": {...}`) across the agent, operator, and TrackerAPI HOCON schemas.
...
4. If `passenger_disembarkation_status` not `'completed'` → call `aircraft_disembark`. Return to step 2 (max 3 retries).
5. If `crew_exit_status` not `'completed'`/`'exited'` → call `aircraft_crew_exit`. Return to step 2.
6. If `baggage_unload_status` not `'completed'`/`'unloaded'` → call `/AirlineTurnaround/aircraft_baggage_unload`. Return to step 2 (max 3 retries).
...
> Note: Steps 4 and 6 include explicit retry guards ("Repeat this loop no more than 3 times"). Step 5 does not currently specify a max retry count.
...
| **To downstream**   | `flight_number`, `aircraft_type`, `gate_id`, `flight_status`, `passenger_disembarkation_status`, `crew_exit_status`, `baggage_unload_status` |
| **From upstream**   | `flight_number`, `aircraft_type`, `gate_id`, `flight_status`, `passenger_disembarkation_status`, `crew_exit_status`, `baggage_unload_status` |
| **From downstream** | `flight_number`, `aircraft_type`, `gate_id`, `flight_status`, `passenger_disembarkation_status`, `crew_exit_status`, `baggage_unload_status` |

> Note: `lavatory_service_status` propagates upstream and via `from_downstream`. It is absent from `to_downstream` and `from_upstream`. Downstream networks therefore cannot receive the lavatory service result via `to_downstream` sly_data.

#### Down-chain tools (HOCON line 225)
...
> Note: The tools list contains five distinct entries with no duplicates. All three external references (`aircraft_disembark`, `aircraft_crew_exit`, `aircraft_baggage_unload`) appear exactly once.
...
> Note: The HOCON TrackerAPI schema exposes many additional turnaround fields (`ground_services_request_type`, `wheels_chocks_*`, `gpu_*`, `acu_*`, `engines_stop_status`, `jetbridge_connection_status`, `door_opening_status`) that are not tracked by the Python config — wide-schema copy-paste artifacts.
...
| Wide-schema HOCON TrackerAPI exposes non-tracked fields          | `aircraft_lavatory_service.hocon` lines 306–357       |   Low    | Copy-paste artifact.                                                                                                                                                                                                                                    |
| Step 5 has no max retry count                                    | `aircraft_lavatory_service.hocon` lines 153–156       |   Low    | Steps 4 and 6 cap retries at 3; step 5 (`aircraft_crew_exit`) does not.                                                                                                                                                                                 |
...
| Duplicate tool in tools list               | No                                | No                               |
| `crew_exit_status` quoted in HOCON         | —                                 | Yes (3 places)                   |
