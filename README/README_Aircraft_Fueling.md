...
| **From downstream** | `flight_number`, `aircraft_type`, `gate_id`, `flight_status`, `passenger_disembarkation_status`, `crew_exit_status`, `baggage_unload_status`, `fueling_status` |
|---------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
> Note: `flight_status` is not declared `required` in the HOCON schema for `fueling_operator` (it matches the `fueling_agent` schema in this respect). It is still passed in by the orchestrator in Step 4.
