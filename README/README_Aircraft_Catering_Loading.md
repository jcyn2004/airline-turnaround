...
> Note: `catering_loading_status` is the value this network produces; it now appears only in `to_upstream`. The other three directions carry inputs (flight identity + disembarkation/crew-exit/baggage prerequisites) but not the catering status itself.
