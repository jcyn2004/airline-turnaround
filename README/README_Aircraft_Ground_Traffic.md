...
|---------------------|----------------------------------------------------------------------------------------------------------------------------------------|
| **To upstream**     | `ground_clearance_type`, `ground_clearance_status`                                                                                     |
| **To downstream**   | `flight_number`, `aircraft_type`, `gate_id`, `flight_status`, `assigned_runway_id`, `ground_clearance_type`                            |
| **From upstream**   | `flight_number`, `aircraft_type`, `gate_id`, `flight_status`, `assigned_runway_id`, `ground_clearance_type`                            |
| **From downstream** | `flight_number`, `aircraft_type`, `gate_id`, `ground_clearance_type`, `ground_clearance_status`                                        |
...
> Note: The `ground_traffic_controller` HOCON definition includes a `parameters` block (lines 234–267) declaring the same 7 properties as the orchestrator, with `aircraft_type`, `flight_number`, `flight_status`, `assigned_runway_id`, `gate_id`, and `ground_clearance_type` listed as required. This gives the LLM at this level an explicit schema for the fields to expect.
...
| Interactive user-wait loop in step 3                                              | `aircraft_ground_traffic.hocon` lines 150–153 |   Low    | If `ground_clearance_type` is None, the agent waits for user response — same risk as `aircraft_engines_stop` in automated contexts.                                                                                   |
