...
> Note: `flight_status` and `gate_id` are absent from the HOCON `required` array even though the orchestrator instructions treat `flight_status` as a hard gate (step 2). This is consistent across the network — `gate_id` is required by the operator but not declared required at the agent level.

#### Orchestration flow

The orchestrator uses explicit numbered steps in its instruction set and is instructed to execute them strictly in sequence:

1. **Collect parameters** — read from inquiry and call `TrackerAPI` to fill in any missing values from `sly_data`.
2. **Verify flight status** — `flight_status` must contain `on blocks` or `block`. If not, stop and report.
3. **Verify passenger disembarkation** — if `passenger_disembarkation_status` does not contain `completed`, call `/AirlineTurnaround/aircraft_disembark`. Return to step 1.
4. **Verify crew exit** — if `crew_exit_status` does not contain `completed` or `exited`, call `/AirlineTurnaround/aircraft_crew_exit`. Return to step 1.
5. **Verify baggage unload** — if `baggage_unload_status` does not contain `completed` or `unloaded`, call `/AirlineTurnaround/aircraft_baggage_unload`. Return to step 1.
6. **Execute cabin cleaning** — call `cabin_cleaning_operator` with all parameters. Store `cabin_cleaning_status`.
7. **Persist state** — call `TrackerAPI` to store `cabin_cleaning_status`.
8. **Return summary**.
...
> Note: The HOCON `to_upstream` block lists `cabin_cleaning_status` twice. The duplicate is harmless but should be removed during schema maintenance.
...
All seven parameters are hard-required by the operator. If any is missing from both `args` and `sly_data`, the tool returns an error string immediately.
...
Manages shared turnaround state. Called at the start of the workflow to read current values, and again after `cabin_cleaning_operator` to persist the updated status. The `TrackerAPI` in this network has a notably **wider HOCON schema** than the Python default configuration — it tracks many additional fields (`wheels_chocks_*`, `gpu_*`, `acu_*`, `jetbridge_*`, `door_opening_status`, etc.) inherited from a broad turnaround template, even though the Python default config only tracks and returns the fields directly relevant to cabin cleaning.
...
|-------------------------------------------------------|--------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Duplicate `cabin_cleaning_status` in `to_upstream`    | `aircraft_cabin_cleaning.hocon` → `allow.to_upstream`                    | Listed twice; safe to remove the duplicate.                                                                                                                                                                                                                                                |
| `gate_id` not in `FLIGHT_TURNAROUND_TRACKED_FIELDS`   | `aircraft_cabin_cleaning.py`                                             | `gate_id` is required by `cabin_cleaning_operator` but TrackerAPI will not track or return it; if lost from `sly_data` it must come from `args` each call. Consider adding to tracked fields.                                                                                              |
| Log message references wrong operation                | `aircraft_cabin_cleaning.py` → `cabin_cleaning_operator.invoke` line 164 | Message reads `"has inspection and maintenance {cabin_cleaning_status}. Its inspection and maintenance status is..."` — should reference cabin cleaning, not inspection and maintenance. Copy-paste artifact from another operator.                                                        |
| Hardcoded log path comment                            | `aircraft_cabin_cleaning.py` → `cabin_cleaning_operator.invoke`          | Commented-out absolute path remains; active path uses `Path.cwd()`.                                                                                                                                                                                                                        |
| HOCON `TrackerAPI` schema vs Python defaults mismatch | `aircraft_cabin_cleaning.hocon`                                          | The HOCON schema exposes many fields from a broad turnaround template (`wheels_chocks_*`, `gpu_*`, `acu_*`, etc.) that are not in `FLIGHT_TURNAROUND_TRACKED_FIELDS`. The Python config is the effective definition; the HOCON schema is wider than needed and should be trimmed to match. |
| Commented-out `required` fields on agent              | `aircraft_cabin_cleaning.hocon` line 135                                 | `passenger_disembarkation_status`, `crew_exit_status`, `baggage_unload_status` are commented out of the HOCON `required` array; the orchestrator treats them as mandatory in its instructions. This is intentional (they come from `sly_data`) but worth noting for clarity.               |
|-------------------------------------------------------|--------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
