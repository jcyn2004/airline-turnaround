...
- **Context forwarding:** The three human-clearance status fields (`passenger_disembarkation_status`, `crew_exit_status`, `baggage_unload_status`) are passed by the caller and are accepted on the inbound (`from_upstream`, `from_downstream`) sly_data allow blocks, although they are not propagated outward through `to_upstream` / `to_downstream`.
...
> Note: These limits match the standard used across the other networks in the system (`max_iterations: 40000` and `max_execution_seconds: 7200`). The HOCON keeps the prior values (`3000` / `600`) as inline comments for reference.
...
> Note: The agent is named `cabin_services` in the HOCON. It has no `function.parameters` wrapper — the `parameters` block is defined directly under the tool alongside `function`, which is the correct neuro-san schema structure.
...
The four directions carry differing field sets:

| Direction         | Fields                                                                                                                                                                                                                      |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `to_upstream`     | `flight_number`, `aircraft_type`, `flight_status`, `gate_id`, `catering_loading_status`, `cabin_cleaning_status`, `lavatory_service_status`                                                                                 |
| `from_upstream`   | `flight_number`, `aircraft_type`, `flight_status`, `gate_id`, `catering_loading_status`, `cabin_cleaning_status`, `lavatory_service_status`, `passenger_disembarkation_status`, `crew_exit_status`, `baggage_unload_status` |
| `from_downstream` | `flight_number`, `aircraft_type`, `flight_status`, `gate_id`, `catering_loading_status`, `cabin_cleaning_status`, `lavatory_service_status`, `passenger_disembarkation_status`, `crew_exit_status`, `baggage_unload_status` |
| `to_downstream`   | `flight_number`, `aircraft_type`, `flight_status`, `gate_id`, `catering_loading_status`, `cabin_cleaning_status`, `lavatory_service_status`                                                                                 |

> Note: `passenger_disembarkation_status`, `crew_exit_status`, and `baggage_unload_status` are accepted inbound (`from_upstream`, `from_downstream`) but are **absent from the outbound (`to_upstream`, `to_downstream`) sly_data allow blocks**. These prerequisite status fields will not propagate outward through sly_data from this network.
...
**Class:** `AirlineTurnaround.aircraft_cabin_services.aircraft_cabin_services.TrackerAPI`

> Note: The class path correctly references the `aircraft_cabin_services` module, matching the Python file `aircraft_cabin_services.py` that contains the `TrackerAPI` class implementation for this network.
...
> Note: The TrackerAPI HOCON schema exposes 5 fields (`aircraft_type`, `gate_id`, `catering_loading_status`, `cabin_cleaning_status`, `lavatory_service_status`) — the three cabin service status fields plus `aircraft_type` and `gate_id`. The HOCON schema is aligned with the cabin-services domain. The Python tracker, however, accepts and returns the full 7-field set including `flight_number` and `flight_status`.
...
```
# Sample query 1 (lavatory service)
...
# Sample query 2 (cabin cleaning)
"The B747 aircraft of flight AF84 is on blocks at gate A1.
Baggages have been unloaded. All passengers have disembarked.
The crew has exited the aircraft. Perform aircraft cabin cleaning and provide a summary report."

# Sample query 3 (catering loading)
"The B747 aircraft of flight AF84 is on blocks at gate A1.
Baggages have been unloaded. All passengers have disembarked.
The crew has exited the aircraft. Load catering to the aircraft."
...
| Execution limits            | 40,000 / 7,200s               | 40,000 / 7,200s                                                   |
| HOCON TrackerAPI class path | Correct module                | Correct module                                                    |
|-----------------------------|-------------------------------|-------------------------------------------------------------------|
...
- If `passenger_disembarkation_status`, `crew_exit_status`, and `baggage_unload_status` need to propagate outward, add them to the `to_upstream` and `to_downstream` sly_data allow blocks
- Consider adding `flight_number` and `flight_status` to the HOCON TrackerAPI parameter schema so the HOCON declaration matches the 7 fields the Python tracker actually handles
- Remove the ~140-line commented-out `execute_aircraft_landing` block from the Python file (if still present)
- Remove the three iterations of commented-out ground readiness instructions from the HOCON (if still present)
- Consider migrating TrackerAPI from args-first to sly_data-first resolution to align with the newer-generation networks in the system
