...
The `to_downstream` and `to_upstream` directions carry an 11-field set (includes `aircraft_direction`):

`aircraft_type`, `flight_number`, `flight_status`, `aircraft_direction`, `ground_clearance_type`, `ground_clearance_status`, `gate_id`, `assigned_runway_id`, `acu_readiness_status`, `gpu_readiness_status`, `wheels_chocks_readiness_status`

The `from_downstream` and `from_upstream` directions carry a 10-field set (no `aircraft_direction`):
