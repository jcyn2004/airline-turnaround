...
3. If `flight_status` contains `on blocks` → skip to step 6.
4. If `flight_status` does not contain `on blocks`:
   - Report to user that engines cannot be stopped while not on blocks.
   - **Wait for user response.**
   - Return to step 2.
5. Call `TrackerAPI` again — re-read `flight_status`. If still not `on blocks` → return to step 4.
6. `flight_status` contains `on blocks` → call `engines_stop_operator`. Wait for response. Save as `engines_stop_status`. Report status.
7. Call `TrackerAPI` to read and confirm `flight_status` and `engines_stop_status`.
8. Return summary.
...
> Note: Step 5 is a redundant TrackerAPI call immediately after step 2. If the aircraft is on blocks and the flow reached step 5, it simply re-confirms what was already confirmed in step 3. This step adds no functional value in the normal path and only serves as a re-check guard before the operator call.
...
Manages shared turnaround state. Called in step 2 to read `flight_status`, again in step 5 as a re-check guard, and again in step 7 after the operator to confirm the final status.
...
**Return fields:**
`engines_stop_status`, `flight_status`
...
3. `engines_stop_operator` called — returns `engines_stop_status=stopped` (step 6)
4. `TrackerAPI` called (step 7) — confirms `engines_stop_status=stopped`, `flight_status=on blocks`
5. Summary returned
