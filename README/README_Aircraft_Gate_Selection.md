...
registries/aaosa_basic_debug.hocon   # Shared registry (debug variant; no external tools used)
```

> Note: This network includes `registries/aaosa_basic_debug.hocon` rather than the `registries/aaosa_basic.hocon` used by all other networks.
...
|---|---|
| **To upstream** | `gate_id`, `flight_number`, `aircraft_type`, `deplaning_equipment_type`, `deplaning_equipment_id`, `deplaning_equipment_score` |
| **From upstream** | `gate_id`, `flight_number`, `aircraft_type`, `deplaning_equipment_type`, `deplaning_equipment_id`, `deplaning_equipment_score` |

> Note: This network has no `to_downstream` or `from_downstream` sly_data blocks — it is intended as an early-stage upstream network that feeds context to the rest of the turnaround pipeline. `deplaning_equipment_readiness_time` is returned by `deplaning_path_selector` and shown in the summary but is absent from the `to_upstream` sly_data block.
...
**Class:** `coded_tools.AirlineTurnaround.aircraft_gate_selection.aircraft_gate_selection.deplaning_path_selector`
...
> Note: The class path uses the `coded_tools.` prefix. All other coded tools in the system use `AirlineTurnaround.` as the root prefix. This difference may reflect a different registration or installation path for this network.
...
**Class:** `coded_tools.AirlineTurnaround.aircraft_gate_selection.aircraft_gate_selection.TrackerAPI`
...
> Note: The class path uses the `coded_tools.` prefix, consistent with `deplaning_path_selector`.
...
> Note: The HOCON `TrackerAPI` description contains two errors: `"statusengines_stop_status"` (concatenated with `status` prefix) and `"wheels_chucks_installation_status"` (typo: "chucks" not "chocks"). Both are stale copy-paste artifacts — neither field is tracked by this network's Python config.
...
|---|---|---|
| Agent name mismatch with prior documentation | `aircraft_gate_selection.hocon` line 91 | Agent is `gate_selection_agent`, not `aircraft_gate_selection_agent` as previously documented. |
...
| `unit_id` typo in CSV | `gate_equipments_base.csv` row 51 | Unit ID is `stairtruck_1o` (letter 'o') instead of `stairtruck_10`. Does not affect selection logic since the unit is identified by its row index, but will return the incorrect ID string in results. |
| `deplaning_equipment_readiness_time` absent from `to_upstream` sly_data | `aircraft_gate_selection.hocon` line 151 | The field is written to sly_data by the operator and shown in the summary, but not listed in the `to_upstream` allow block. Upstream networks reading this value via sly_data may not receive it. |
| HOCON TrackerAPI description has stale copy-paste artifacts | `aircraft_gate_selection.hocon` lines 429–431 | Description references `"statusengines_stop_status"` (concatenated prefix) and `"wheels_chucks_installation_status"` (typo). Neither field is tracked by this network. |
| `coded_tools.` class path prefix differs from all other networks | HOCON lines 219, 464 | All other networks use `AirlineTurnaround.` as root. This network uses `coded_tools.AirlineTurnaround.`. Ensure the installation/registration path matches. |
| Commented-out sub-agents represent stale architecture | `aircraft_gate_selection.hocon` and `.py` | Four deprecated tools (`jetway_agent`, `stairtruck_agent`, `jetway_park_agent`, `stairtruck_park_agent`) and an alternative `TrackerAPI` implementation (lines 450–1309 in Python) are fully commented out. These represent ~900 lines of dead code that could be removed or archived. |
| `aaosa_basic_debug.hocon` vs `aaosa_basic.hocon` | `aircraft_gate_selection.hocon` line 3 | This network uses the debug registry variant. Confirm whether debug mode is intentional for production deployments. |
...
- Add `deplaning_equipment_readiness_time` to the `to_upstream` sly_data block
- Remove or archive the ~900 lines of commented-out code (deprecated sub-agent architecture)
- Fix the `stairtruck_1o` typo in the CSV
- Generalize beyond B747 by adding entries for other aircraft types to `gate_equipments_base.csv`
- Normalize the class path prefix (`coded_tools.AirlineTurnaround.` → `AirlineTurnaround.`) for consistency with other networks, if the deployment environment permits
