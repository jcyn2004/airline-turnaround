...
| **To downstream**   | `flight_number`, `aircraft_type`, `flight_status`, `gate_id`, `jetbridge_connection_status`, `stairtruck_connection_status`, `deplaning_equipment_type`, `door_opening_status`, `baggage_unload_status` |
| **From upstream**   | `flight_number`, `aircraft_type`, `flight_status`, `gate_id`, `jetbridge_connection_status`, `stairtruck_connection_status`, `deplaning_equipment_type`, `door_opening_status`, `baggage_unload_status` |
| **From downstream** | `flight_number`, `aircraft_type`, `flight_status`, `gate_id`, `jetbridge_connection_status`, `stairtruck_connection_status`, `deplaning_equipment_type`, `door_opening_status`, `baggage_unload_status` |
|---------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|

> Note: The HOCON defines `to_downstream` twice — an initial narrow block (`baggage_unload_status` only) and a full block below it. The second definition overrides the first. The full block is the effective one.
...
|---------------------------------------------------------------|--------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------|
| Duplicate `to_downstream` block                               | `aircraft_baggage_unload.hocon` → `allow` section                        | First narrow definition (only `baggage_unload_status`) is overridden by the second full definition. Remove the first to avoid confusion.    |
| `baggage_unload_status` not in HOCON `TrackerAPI` description | `aircraft_baggage_unload.hocon` → `TrackerAPI` description               | The description only mentions `jetbridge_connection_status` and `door_opening_status`; does not mention `baggage_unload_status`, `stairtruck_connection_status`, or `deplaning_equipment_type`, all of which are tracked. Description should be updated. |
| Log message typo                                              | `aircraft_baggage_unload.py` → `baggage_unload_operator.invoke` line 150 | Message contains `"installed. Its baggage unload status is status is"` — duplicated `"status is"`.                                          |
| Hardcoded log path comment                                    | `aircraft_baggage_unload.py` → `baggage_unload_operator.invoke`          | Commented-out absolute path remains; active path uses `Path.cwd()`.                                                                         |
