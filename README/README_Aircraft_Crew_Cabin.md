...
| `flight_status`                   | string |    Yes   | Expected: contains `on blocks`    |
| `flight_number`                   | string |    Yes   | Flight identifier                 |
| `gate_id`                         | string |    No    | Gate where the aircraft is parked |
| `task_id`                         | string |    No    | **Primary routing discriminant**  |
| `deplaning_equipment_type`        | string |    No    | `jetway` or `stairtruck`          |
| `jetbridge_connection_status`     | string |    No    | Required for BRANCH A             |
| `stairtruck_connection_status`    | string |    No    | Required for BRANCH A             |
| `acu_connection_status`           | string |    No    | Required for BRANCH A             |
| `gpu_connection_status`           | string |    No    | Required for BRANCH A             |
| `wheels_chocks_installation_status` | string |   No    | Required for BRANCH A             |
| `door_opening_status`             | string |    No    | Required for BRANCH B + C         |
| `passenger_disembarkation_status` | string |    No    | Required for BRANCH C             |
| `baggage_unload_status`           | string |    No    | Required for BRANCH C             |
| `crew_debrief_status`             | string |    No    | Output of BRANCH C                |
| `assigned_runway_id`              | string |    No    | Contextual field                  |
|-----------------------------------|--------|:--------:|-----------------------------------|
...
|---------------------|------------|
| **To upstream**     | `door_opening_status`, `passenger_disembarkation_status`, `crew_debrief_status`, `task_id` |
| **From upstream**   | `flight_number`, `aircraft_type`, `gate_id`, `flight_status`, `task_id`, `deplaning_equipment_type`, `jetbridge_connection_status`, `stairtruck_connection_status`, `acu_connection_status`, `gpu_connection_status`, `wheels_chocks_installation_status`, `door_opening_status`, `passenger_disembarkation_status`, `baggage_unload_status` |
| **To downstream**   | `flight_number`, `aircraft_type`, `gate_id`, `flight_status`, `deplaning_equipment_type`, `jetbridge_connection_status`, `stairtruck_connection_status`, `acu_connection_status`, `gpu_connection_status`, `wheels_chocks_installation_status`, `door_opening_status`, `passenger_disembarkation_status`, `baggage_unload_status` |
| **From downstream** | `flight_number`, `aircraft_type`, `gate_id`, `flight_status`, `door_opening_status`, `passenger_disembarkation_status`, `crew_debrief_status` |

> Note: `task_id` flows between this network and its caller (`to_upstream` and `from_upstream`) as a correlation token; it is not forwarded to downstream sub-networks in the refactored contract.
...
> Note: The HOCON TrackerAPI free-text description mentions `engines_stop_status` and `wheels_chucks_installation_status` (double-`c` typo). Neither field appears in the HOCON TrackerAPI parameter schema, the sly_data allow blocks, or the Python `FLIGHT_TURNAROUND_TRACKED_FIELDS`. Stale copy-paste artifact also seen in several other networks. Correct field name is `wheels_chocks_installation_status`.
...
The HOCON `sample_queries` array contains two natural-language queries (BRANCH A and BRANCH B):

```
# BRANCH A — door opening
...
"The B747 aircraft of flight AF84 is on blocks at gate A1.
 The jetbridge is connected to the plane. The aircraft door is open.
