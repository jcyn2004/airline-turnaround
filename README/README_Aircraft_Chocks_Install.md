...
- **sly_data as shared state:** Both tools communicate through `sly_data` — the installation result flows to upstream networks without re-passing through the LLM.
...
| **From upstream** | `flight_number`, `aircraft_type`, `flight_status`, `gate_id`, `wheels_chocks_installation_status` |

> Note: This network has no `to_downstream` or `from_downstream` sly_data blocks defined in the HOCON. `wheels_chocks_installation_status` is only propagated upstream. Downstream networks that need this value must read it from upstream sly_data or call this network directly.
...
- Add `to_downstream` and `from_downstream` sly_data blocks so `wheels_chocks_installation_status` is available to downstream networks (particularly `aircraft_acu_connect` which explicitly requires it)
- Align HOCON TrackerAPI field names with Python (`wheelchocks_*` → `wheels_chocks_*`)
