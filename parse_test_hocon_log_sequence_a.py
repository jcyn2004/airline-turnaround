# New code version created on 20260127 
...
def get_operation_code(operation_name):
    ...
    Returns:
        str: The operation code, or '00' if not found
    """
    return aircraft_operations.get(operation_name, '00')

# Determine when a list contains at least one non None element (excluding first 3 fixed columns)
def has_non_none_element(lst):
    # Check if there's any non-None value after the first 3 positions (timestamp, agent_name, step_code)
    return any(x is not None for x in lst[3:])

df = pd.DataFrame(columns=column_names)

# Create a dictionary to accumulate data across multiple DATA SUMMARY blocks
accumulated_data = {}
...
# Variable to track current timestamp for grouping
...
with open('test_hocon_log.txt', 'r') as file:
    for line in file:
        ...
        if (data_summary_tag_end in line):
            ...
            # When we exit a DATA SUMMARY block, save the accumulated data if it has content
            if current_timestamp and has_non_none_element(list(accumulated_data.values())):
                # Create a new row from accumulated data
                row_data = [None] * len(column_names)
                row_data[0] = current_timestamp
                row_data[1] = current_agent
                row_data[2] = current_agent_code
                
                # Fill in the accumulated parameter values
                for param, value in accumulated_data.items():
            ...
            # Reset accumulated data for next block
            accumulated_data = {}
            current_timestamp = None
            current_agent = None
            current_agent_code = None
        ...
        if (data_summary_block == 1): 
            # Check if line contains required patterns
            ...
            if dash_count == 3 and info_count == 1 and pipe_count == 2:
                ...
                if len(parts) >= 3:
                    # Extract timestamp and agent info (only once per block)
                    if current_timestamp is None:
                        current_timestamp = parts[0]
                        agent_full = parts[1]
                        
                        # Extract agent name from the full path
                        # Example: coded_tools.AirlineTurnaround.aircraft_turnaround.aircraft_turnaround
                        agent_parts = agent_full.split('.')
                        # if len(agent_parts) >= 3:
                        #     current_agent = agent_parts[2]  # e.g., aircraft_turnaround
                        # else:
                        #     current_agent = agent_full

                        if len(agent_parts) >= 2:
                            current_agent = agent_parts[-1]  # e.g., aircraft_turnaround
                        else:
                            current_agent = agent_full

                        # Get the agent step operational code 
                        current_agent_code = get_operation_code(current_agent)
                    ...
                    if len(data_parts) >= 3:
                        ...
                        accumulated_data[parameter] = value
                        
                        # # Only accumulate if the value is not 'None' or 'UNSET'
                        # if value and value != 'None' and value != '[RETURN]':
                        #     # Store in accumulated_data dictionary
...
print(f"Columns with data: {df.notna().sum().sum()}")
