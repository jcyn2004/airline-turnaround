...
"""
import nest_asyncio
nest_asyncio.apply()  # Must run before any event loop is created. Fixes the
                      # "bound to a different event loop" RuntimeError caused by
                      # LangGraph's asyncio.create_task() conflicting with
                      # langchain_anthropic's _async_client, which gets created
                      # on a different loop at ChatAnthropic instantiation time.
