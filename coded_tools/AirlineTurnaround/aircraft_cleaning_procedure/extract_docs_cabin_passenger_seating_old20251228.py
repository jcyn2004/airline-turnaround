...
class ExtractDocs(CodedTool):
    ...
    def invoke(self, args: Dict[str, Any], sly_data: Dict[str, Any]) -> Union[Dict[str, Any], str]:
        ...
        user_inquiry_compliant: str = args.get("user_inquiry_compliant", None)
        print("\n")
        print("\n")
        print("user_inquiry_compliant in args: ",user_inquiry_compliant)
        print("\n")
        print("\n")
        if not user_inquiry_compliant:
            print("user inquiry compliant not provided. Trying to get it from sly_data")
            user_inquiry_compliant = sly_data.get("user_inquiry_compliant")
            print("\n")
            print("\n")
            print("user_inquiry_compliant in sly_data: ",user_inquiry_compliant)
            print("\n")
            print("\n")   
        if sly_data.get("user_inquiry_compliant", None) is None:
            sly_data["user_inquiry_compliant"] = user_inquiry_compliant
        ...
        if (((response_to_recommendation == 'yes') & (restriction_applies == 'yes')) | (restriction_applies == 'no') | (user_inquiry_compliant == 'yes')):
        ...
        return compliant_cleaning, cabin_cleaning_status
...
class pull_restriction(CodedTool):
    ...
    def invoke(self, args: Dict[str, Any], sly_data: Dict[str, Any]) -> Union[Dict[str, Any], str]:
        ...
        #     error = "Error: Please provide a ground clearance type for the inquiry."
        #     print(error)
        #     return error    
        sly_data["restriction_applies"] = restriction_applies
...
class create_escalation(CodedTool):
    ...
    def invoke(self, args: Dict[str, Any], sly_data: Dict[str, Any]) -> Union[Dict[str, Any], str]:
        ...
        # ground clearance type is required to fulfill the inquiry.
        restriction_on_cleaning: str = args.get("restriction_on_cleaning", None)
        print("\n")
        print("\n")
        print("restriction_on_cleaning in args: ",restriction_on_cleaning)
...
class cleaning_tracker(CodedTool):
    ...
    def invoke(self, args: Dict[str, Any], sly_data: Dict[str, Any]) -> Union[Dict[str, Any], str]:
        ...
        if (response_to_recommendation == 'yes'): 
            user_inquiry_compliant = 'yes'
...
class final_response(CodedTool):
    ...
    def invoke(self, args: Dict[str, Any], sly_data: Dict[str, Any]) -> Union[Dict[str, Any], str]:
        ...
        cabin_cleaning_status = "in progress"
        ...
        if not response_to_recommendation:
            print("No response to recommendation provided. Trying to get it from sly_data")
            response_to_recommendation = sly_data.get("response_to_recommendation")
            print("response_to_recommendation from sly_data:", response_to_recommendation)
        if sly_data.get("response_to_recommendation", None) is None:
            sly_data["response_to_recommendation"] = response_to_recommendation
        ...
        if (('yes' in response_to_recommendation) | ('Yes' in response_to_recommendation)): 
            cabin_cleaning_status = "completed"

        return final_response, cabin_cleaning_status
