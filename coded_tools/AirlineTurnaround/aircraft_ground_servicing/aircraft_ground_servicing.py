...
class TrackerAPI(CodedTool):
    ...
    def _process_field(
        ...
        """
        Process a single field by checking args first, then falling back to sly_data.

        Priority:
          1. args[field_name]     - explicit write always wins; value is
             stored into sly_data so subsequent calls find it there.
          2. sly_data[field_name] - read existing state when args has no
             value for this field.
          3. Neither source       - returns (None, NOT_FOUND).

        Rationale: args carries the *current* operation's output (e.g.
        fueling_status from the fueling agent). sly_data carries *prior*
        state. If we always preferred sly_data, a completed inspection from
        STEP 18 would silently block the fueling write in STEP 19, causing
        the agent to return the wrong branch summary.

        Args:
            field_name: Name of the field to process
            args: Input arguments for the current invocation; consulted first
            sly_data: Shared data store containing accumulated state

        Returns:
            Tuple of (field_value, data_source)
        """
        # 1. args is authoritative for writes — new values always propagate
        ...
        # 2. Fall back to existing sly_data state
