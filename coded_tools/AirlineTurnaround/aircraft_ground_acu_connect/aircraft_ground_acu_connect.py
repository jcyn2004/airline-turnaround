...
class acu_operator(CodedTool):
    ...
    def invoke(self, args: Dict[str, Any], sly_data: Dict[str, Any]) -> Union[Dict[str, Any], str]:
        ...
        acu_readiness_status: str = sly_data.get("acu_readiness_status", None)
        if not acu_readiness_status:
            print("No acu readiness status provided in sly_data. Trying to get it from args")
            acu_readiness_status = args.get("acu_readiness_status")
        if not acu_readiness_status:
