...
class deplaning_path_selector(CodedTool):
    """
    ...
    # def get_best_gate(equipments_csv_path: str, aircraft_type: str) -> dict:
        """
        Finds the most ready gate equipment (jetway or stairtruck) for a given aircraft type.

        Args:
            file_path: Path to the CSV file.
            aircraft_type: Aircraft type to filter on (e.g. 'B747').

        Returns:
            A dict with gate_id, unit_id, type, readiness, and score of the best available unit.
        """

        # Prefer args 
        aircraft_type: Optional[str] = args.get("aircraft_type")

        # Fallback to sly_data if missing args
        if not aircraft_type:
            aircraft_type = sly_data.get("aircraft_type")

        df = pd.read_csv(self.equipments_csv_path)

        # Step 0: Filter by aircraft type and compute score
        df = df[df["aircraft_type"] == aircraft_type].copy()

        if df.empty:
            raise ValueError(f"No entries found for aircraft type: {aircraft_type}")

        df["score"] = 1 / (1 + df["readiness"])

        # Step 1: Best jetway (lowest readiness = highest score)
        jetways = df[(df["type"] == "jetway") & (df["availability"] == "yes")]
        if jetways.empty:
            jetway_dict = None
        else:
            best_jetway = jetways.loc[jetways["readiness"].idxmin()]
            jetway_dict = best_jetway[["gate_id", "unit_id", "type", "readiness", "score"]].to_dict()

        # Step 2: Best stairtruck (lowest readiness = highest score)
        stairtrucks = df[(df["type"] == "stairtruck") & (df["availability"] == "yes")]
        if stairtrucks.empty:
            stairtruck_dict = None
        else:
            best_stairtruck = stairtrucks.loc[stairtrucks["readiness"].idxmin()]
            stairtruck_dict = best_stairtruck[["gate_id", "unit_id", "type", "readiness", "score"]].to_dict()

        # Step 3: Pick the one with the smallest readiness
        candidates = [d for d in [jetway_dict, stairtruck_dict] if d is not None]

        if not candidates:
            raise ValueError("No jetway or stairtruck found for the given aircraft type.")

        gate_dict = min(candidates, key=lambda d: d["readiness"])
        gate_id, deplaning_equipment_id, deplaning_equipment_type, deplaning_equipment_readiness_time, deplaning_equipment_score = gate_dict["gate_id"], gate_dict["unit_id"], gate_dict["type"], gate_dict["readiness"], gate_dict["score"]

        sly_data["gate_id"] = gate_id
        sly_data["deplaning_equipment_id"] = deplaning_equipment_id
        sly_data["deplaning_equipment_type"] = deplaning_equipment_type
        sly_data["deplaning_equipment_readiness_time"] = deplaning_equipment_readiness_time
        sly_data["deplaning_equipment_score"] = deplaning_equipment_score

        df.loc[df["unit_id"] == gate_dict["unit_id"], "availability"] = "no"
    ...
    if __name__ == "__main__":
        result = get_best_gate("gate_equipments_base.csv", "B747")
